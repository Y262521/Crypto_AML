  ...
  filteredData.forEach((tx, index) => {
    ...
    if (tx.sender && tx.receiver && tx.receiver !== 'Contract Creation') {
      ...
      edgesArray.push({
        ...
        labelBgStyle: { fill: '#0D1628', fillOpacity: 0.85 },
      });
    }
  });
  ...
  if (options.collapseLeaves || options.maxNodes) {
    const simplified = collapseLowActivityNodes({
      ...
      createSummaryEdge: (id, hub, count) => ({
        ...
        labelStyle: { fontSize: '9px', fill: '#8A9DB5', fontWeight: '600' },
...
const AddressPanel = ({ address, stats, edgesData, onClose }) => {
  ...
  const renderOwnerSection = () => {
    ...
    const categoryColors = {
      sanction: { color: '#F87171', background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.28)' },
      watchlist: { color: '#FBBF24', background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.28)' },
      exchange: { color: '#60A5FA', background: 'rgba(96,165,250,0.12)', border: '1px solid rgba(96,165,250,0.28)' },
      merchant: { color: '#4ADE80', background: 'rgba(74,222,128,0.12)', border: '1px solid rgba(74,222,128,0.28)' },
    };
    const pillStyle = (key) => ({
      ...(categoryColors[key] || { color: '#8A9DB5', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)' }),
      borderRadius: '999px',
      display: 'inline-flex',
      alignItems: 'center',
      padding: '2px 8px',
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ padding: '16px 18px', borderBottom: '1px solid rgba(201,168,76,0.10)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: '12px', color: '#6B7E94', marginBottom: '4px' }}>
            {stats.isHighValue ? 'High-value address' : 'Address'}
          </div>
          <div style={{ fontSize: '11px', fontFamily: 'monospace', color: '#C9A84C', wordBreak: 'break-all' }}>
            {address}
          </div>
        </div>
        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8A9DB5', fontSize: '18px', marginLeft: '8px', flexShrink: 0 }}>✕</button>
      </div>

      {/* Owner section */}
      <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(201,168,76,0.08)' }}>
        <div style={{ fontSize: '11px', fontWeight: '600', color: '#4B5E72', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>
          Owner
        </div>
        {renderOwnerSection()}
      </div>
      ...
      <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(201,168,76,0.08)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {[
            ...
            <div key={label} style={{ background: 'rgba(201,168,76,0.05)', borderRadius: '8px', padding: '10px', border: '1px solid rgba(201,168,76,0.10)' }}>
              <div style={{ fontSize: '10px', color: '#4B5E72', marginBottom: '3px' }}>{label}</div>
              <div style={{ fontSize: '13px', fontWeight: '700', color: '#C8B98A' }}>{value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Links */}
      <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(201,168,76,0.08)', display: 'flex', gap: '8px' }}>
        <a href={`https://etherscan.io/address/${address}`} target="_blank" rel="noreferrer"
          style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'rgba(96,165,250,0.10)', color: '#60A5FA', borderRadius: '7px', fontSize: '12px', fontWeight: '600', textDecoration: 'none', border: '1px solid rgba(96,165,250,0.25)' }}>
          🔍 Etherscan
        </a>
        <a href={`https://etherscan.io/address/${address}#tokentxns`} target="_blank" rel="noreferrer"
          style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'rgba(74,222,128,0.10)', color: '#4ADE80', borderRadius: '7px', fontSize: '12px', fontWeight: '600', textDecoration: 'none', border: '1px solid rgba(74,222,128,0.25)' }}>
          🪙 Tokens
        </a>
      </div>
      ...
      <div style={{ padding: '12px 18px', flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
          <div style={{ fontSize: '11px', fontWeight: '600', color: '#4B5E72', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Recent Transactions
          </div>
          {hasMoreTransactions && (
            <button
              ...
              style={{
                padding: '5px 10px', borderRadius: '8px', border: '1px solid rgba(201,168,76,0.15)',
                background: 'transparent', color: '#C9A84C', cursor: 'pointer', fontSize: '11px', fontWeight: '700'
              }}
            >
        ...
        {txList.length === 0
          ? <div style={{ color: '#4B5E72', fontSize: '12px' }}>No transactions found</div>
          : (
            <div style={{ marginBottom: '10px', fontSize: '11px', color: '#6B7E94' }}>
        ...
        {txList.length === 0
          ...
          : displayedTx.map((tx, i) => {
            ...
            return (
              <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid rgba(201,168,76,0.06)', fontSize: '11px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                  <span style={{ color: isSender ? '#F87171' : '#4ADE80', fontWeight: '600' }}>
                    {isSender ? '↑ Sent' : '↓ Received'}
                  </span>
                  <span style={{ color: '#4ADE80', fontWeight: '600' }}>{parseFloat(tx.amount).toFixed(6)} ETH</span>
                </div>
                <div style={{ color: '#6B7E94', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 48px)', gap: '12px', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <div style={{ fontSize: '18px', fontWeight: '700', color: '#C8B98A' }}>Transaction Network</div>
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: '8px', flex: 1, maxWidth: '480px' }}>
          <input
            ...
            style={{
              flex: 1, padding: '8px 14px', border: '1px solid rgba(201,168,76,0.18)',
              borderRadius: '8px', fontSize: '13px', outline: 'none',
              background: 'rgba(10,16,32,0.8)', color: '#E2D9C8',
            }}
          />
          <button type="submit" style={{
            padding: '8px 18px', background: 'rgba(201,168,76,0.14)', color: '#C9A84C',
            border: '1px solid rgba(201,168,76,0.30)', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600',
          }}>
            Investigate
          </button>
          {search && (
            <button type="button" onClick={() => {
              ...
              padding: '8px 12px', background: 'transparent', color: '#8A9DB5',
              border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', cursor: 'pointer', fontSize: '12px',
            }}>
              Clear
            </button>
          )}
        ...
        {lastUpdated && (
          <span style={{ fontSize: '11px', color: '#4B5E72' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <label style={{ fontSize: '11px', color: '#8A9DB5', display: 'flex', alignItems: 'center', gap: '6px' }}>
          Hops
          <select value={hops} onChange={e => setHops(Number(e.target.value))}
            style={{ padding: '5px 8px', borderRadius: '6px', border: '1px solid rgba(201,168,76,0.18)', fontSize: '11px', background: 'rgba(10,16,32,0.8)', color: '#E2D9C8' }}>
        ...
        <button type="button" disabled={!submittedSearch} onClick={() => setEdgeCap(v => Math.min(v + 200, 2000))}
          style={{
            padding: '6px 12px', background: !submittedSearch ? 'transparent' : 'rgba(201,168,76,0.08)',
            color: !submittedSearch ? '#4B5E72' : '#C9A84C',
            border: `1px solid ${!submittedSearch ? 'rgba(255,255,255,0.06)' : 'rgba(201,168,76,0.25)'}`,
            borderRadius: '7px', cursor: !submittedSearch ? 'not-allowed' : 'pointer',
      ...
      {/* ── Legend ── */}
      <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#8A9DB5', flexWrap: 'wrap' }}>
        ...
        <span><span style={{ color: '#8A9DB5', fontWeight: '700' }}>●</span> Summary node</span>
        <span style={{ color: '#4B5E72' }}>Drag nodes · Scroll to zoom · Click node for details</span>
      </div>

      {/* ── Graph canvas ── */}
      <div style={{ flex: 1, position: 'relative', borderRadius: '12px', overflow: 'hidden', border: '1px solid rgba(201,168,76,0.12)', background: '#0D1628', minHeight: 0 }}>
        {loading
            ...
            : (
              ...
              >
                ...
                <MiniMap
                  ...
                  style={{ background: '#0D1628', border: '1px solid rgba(201,168,76,0.20)', borderRadius: '8px' }}
                />
              </ReactFlow>
            )
        }
